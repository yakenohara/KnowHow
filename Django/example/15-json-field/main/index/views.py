...
class BookCreate(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        obj_context = super().get_context_data(**kwargs)

        # バリデーションエラーの場合
        if self.request.method == 'POST':

            # https://docs.djangoproject.com/en/4.0/ref/request-response/#django.http.QueryDict.getlist
            for str_tag in self.request.POST.getlist('tags', []):

                # note
                #
                # `obj_context` に `bl_tag_~` というプロパティを追加する理由は後述(以下) のコメント参照
                #
                # `class BookUpdate(UpdateView):`
                #  -> `def get_context_data(self, **kwargs):`
                #  -> `for str_tag in self.object.tags:`
                #
                obj_context['bl_tag_' + str_tag] = True
...
class BookUpdate(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # note
        #
        # `tags` フィールドの widget は `forms.CheckboxSelectMultiple` に設定してあるので、
        # html テンプレート側では以下のように、`tags` フィールドの実態である文字列配列の要素として存在するかどうかという形で実装することはできない。
        #
        # ```
        # <input type="checkbox" ~~~~~~~~~~Omitting~~~~~~~~~~ {% if "for_kids" in form.tags %} checked{% endif %}>
        # ```
        #
        # そのため、`tags` フィールドに対応した個々のチェックボックスについて、
        # チェック = True なもののみ `bl_tag_for_kids` のような html 内の変数を生成する
        #
        for str_tag in self.object.tags:
            obj_context['bl_tag_' + str_tag] = True
